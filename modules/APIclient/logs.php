...
EOD;
		...
		<div id="workarea">
			...
			<div id="workspace" align="center">				
					<?php
						$query = 'select count(aid) from logs where aid="'.$_SESSION['aid'].'"';
						$result = mysql_query($query);
						$rows = mysql_fetch_array($result);
						if(!$rows[0]== 0)
						{
							echo '<p class="tstyle3">The following users logged into your app:</p>';
							echo '<table class="table" style="margin-bottom:10px; width:500px" border="0"><tr><td class="name" style="background-color:#ebebeb;">User</td><td  class="time"style="background-color:#ebebeb;">Time</td><td class="ip" style="background-color:#ebebeb;">IP Address</td></tr>';

							$query = 'select * from logs where aid="'.$_SESSION['aid'].'"';
							$result = mysql_query($query);
