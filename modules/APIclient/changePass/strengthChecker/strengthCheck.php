...
else
{
	...
	if($rows[0] != 0)
	{
		...
		if($rows[0] == 'P')
		{
			...
			$query = "delete from apipicpasswords where id='".$_SESSION['aid']."'";
			mysql_query($query);
			$query  = <<< EOD
			insert into apipicpasswords values('$_SESSION[aid]','$n','$pr','$pt');
EOD;
			mysql_query($query);
			echo '1';
		}
...
EOD;
			...
			$query = "update apipasswords set password_type = 'P' where id = '".$_SESSION['aid']."'";
			mysql_query($query);
			echo '1';
		}
	...
	else
	{
		...
		$query = "insert into apipasswords values('".$_SESSION['aid']."','P')";		
		mysql_query($query);
		$query  = <<< EOD
		insert into apipicpasswords values('$_SESSION[aid]','$n','$pr','$pt');
EOD;
