...
<?php
	...
	$query = "select zooms from mappasswords where id='".$uid."'";
...
function submitPattern()
{
	...
	$.post("userPasswordMatch/mapPassword.php",dataToSend,function(data){
	
		if(data.status == '1')
		{
			//PASSWORD MATCHED
			showNotif("Logging you in...",function(){
				window.opener.location.href=data.redirect+"?token="+data.reqtoken;
				window.close();
			});	
		}
		else
		{
			showDialog("Something went wrong there!",data.status,0);			
		}
	},"json");
}
